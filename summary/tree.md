    ...
    5. 堆的使用场景
            (1) 优先级队列
                    ...
                    b. 高性能定时器
                            ...
                            思路: 按照任务设定的执行时间，将这些任务存储在优先级队列中，队列首部（也就是小顶堆的堆顶）
                                  存储的是最先执行的任务。 这样，定时器就不需要每隔 1 秒就扫描一遍任务列表。它拿队首
                                  任务的执行时间点， 与当前时间点相减，得到一个时间间隔 T。 定时器可以设定在 T 秒之后，
                                  再来执行任务。 从当前时间点到（T-1）秒这段时间里，定时器都不需要做任何事情。
                                 当 T 秒时间过去之后，定时器取优先级队列中队首的任务执行。然后再计算新的队首任务的执行
                                 时间点与当前时间点的差值，把这个值作为定时器执行下一个任务需要等待的时间。
            (2) Top K
            ...
            (3) 求中位数
                    低效方法: 对当前数据进行排序，取中位数，当有数据添加时，再遍历该序列将数据插到指定的位置，
                    　　　　　再算出中位数，时间复杂度为 O(n)
                    高效方案: 初始化的时候也是要先排序，维护两个堆，一个大顶堆(前半数据)，一个小顶堆(后半数据)，
                             需要满足的条件是
                    　　　　　如果 n 是偶数，两个堆中的数据个数都是 n/2；中位数为两个堆顶和除以 2
                             如果 n 是奇数，大顶堆有 n/2 + 1 个数据，小顶堆有 n/2 个数据, 中位数为大顶堆的堆顶元素
                             插入数据时：
                                如果新加入的数据小于等于大顶堆的堆顶元素，就将这个新数据插入到大顶堆；
                                如果新加入的数据大于等于小顶堆的堆顶元素，就将这个新数据插入到小顶堆。
                                这个时候，可以从一个堆中不停地将堆顶元素移动到另一个堆，通过这样的调整，
                                来让两个堆中的数据满足上面的条件。
            (4) 快速求接口的 99% 响应时间
                    99 百分位数的概念可以类比中位数，如果将一组数据从小到大排列，这个 99 百分位数就是大于前面 99% 
                    数据的那个数据. 如果有 100 个接口访问请求，每个接口请求的响应时间都不同，比如 55 毫秒、100 毫秒、
                    23 毫秒等，把这 100 个接口的响应时间按照从小到大排列，排在第 99 的那个数据就是 99% 响应时间，
                    也叫 99 百分位响应时间.
                    具体思路:
                        维护两个堆，一个大顶堆，一个小顶堆。大顶堆中保存 n * 99% 个数据，小顶堆中保存 n * 1% 个数据。
                        大顶堆堆顶的数据就是要找的 99% 响应时间。
                        插入新数据时如果这个新插入的数据比大顶堆的堆顶数据小，那就插入大顶堆；
                        如果这个新插入的数据比小顶堆的堆顶数据大，那就插入小顶堆，这个时候需要重新平衡使之达到上述的要求.
                        
            
    6. 综合应用   
            (1) 一个包含 10 亿个搜索关键词的日志文件，如何快速获取到 Top 10 最热门的搜索关键词
                思路:
                    第一步：先通过散列表、平衡二叉查找树或者其他一些支持快速查找、插入的数据结构，
                    　　　　来记录关键词及其出现的次数，暂时先选用散列表，顺序扫描这 10 亿个搜索关键词。当扫描到某个关键词时，
                    　　　　去散列表中查询。如果存在，就将对应的次数加一；如果不存在，将它插入到散列表，并记录次数为 1。
                    　　　　遍历完这 10 亿个搜索关键词之后，散列表中就存储了不重复的搜索关键词以及出现的次数。
                    第二步: 再构造小顶堆，遍历散列表，依次取出每个搜索关键词及对应出现的次数，然后与堆顶的搜索关键词对比。
                    　　　　如果出现次数比堆顶搜索关键词的次数多，那就删除堆顶的关键词，将这个出现次数更多的关键词加入到堆中
                    
                    后续问题:
                            如果 10 亿个搜索关键词重复的很少(有 1 亿个非重复的关键字)，那么散列表在内存中占用的空间就很大
                            (还得算上装载因子)，这时候 1G 内存是存不下的，解决的方案是利用哈希算法进行数据分片，将 10 亿
                            个关键字通过某个哈希算法对其求哈希值，然后哈希值同 10 取模，得到的结果就是这个搜索关键词应该被
                            分到的文件编号。之后在依次遍历这 10 个文件，建立散列表，构造小顶堆，这样散列表的大小就会减少
                            （根据时间差，先进行第一个文件处理，构造了第一个文件的散列表，之后第二个文件的散列表可以替换掉
                            　第一个文件的散列表）
